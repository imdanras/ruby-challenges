...
def guessing_game
  ...
  while number_guess != random_number do
      ...
      if number_guess < random_number
        puts "The number is greater than #{number_guess}. Guess again"
        count += 1
        number_guess = gets.chomp.to_i

      elsif number_guess >  random_number
        puts "The number is less than #{number_guess}. Guess again"
        count += 1
        number_guess = gets.chomp.to_i

      end

    return puts "You got it in #{count} tries"
